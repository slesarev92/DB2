...
по UI, верстке, логике и расчётам. Выявлено 12 багов, 20 UX-замечаний,
8 логических замечаний и 16 backlog-пожеланий.

**Контекст:** Первый раунд обратной связи от заказчика после демо v0.3.0
на `https://db2.medoed.work`. Заказчик тестировал все разделы: Содержание,
Фин.План, SKU и BOM, Ингредиенты, Каналы, АКБ, OBPPC, Периоды, Сценарии,
Результаты, Чувствительность, P&L, Цены, Стакан, Экспорт.

**Критические баги:**
1. **BUG-01:** Экспорт (XLSX/PPTX/PDF) не работает на проде
2. **BUG-02:** CAPEX нельзя обнулить — вылетает ошибка при вводе 0
3. **BUG-03:** Ошибка при загрузке изображения упаковки
4. **BUG-05:** Периодическая ошибка в сценариях
5. **BUG-06:** GateTimeline не обновляется при смене gate_stage

**Визуальные баги:**
6. **BUG-07:** Верстка при пустом списке SKU — правый блок сдвигается
7. **BUG-08:** «Что-то красное» в BOM — непонятный validation indicator
8. **BUG-09:** Глючная верстка таблицы Ингредиентов
9. **BUG-10:** Overflow длинного названия канала
10. **BUG-11:** Верстка секции КП на производство съехала
11. **BUG-12:** Верстка вокруг AG Grid в Периодах
12. **BUG-04:** Ошибка в OBPPC табе

**Решение:** Полный разбор в `docs/CLIENT_FEEDBACK_v1.md`. План исправлений
по приоритетам. Ожидает одобрения пользователя перед началом работы.

**Урок:** Первый раунд от заказчика всегда выявляет «слепые зоны» разработчика:
- Подписи и единицы измерения — разработчик считает очевидным, заказчик нет
- Копакинг — ключевая бизнес-логика, которая в Excel-модели = 0, но в реальности критична
- Структура P&L — заказчик ожидает точную копию листа DATA, а не абстрактный P&L
- НДС — в FMCG бывает 10%/22%/0%, единый VAT_RATE недостаточен для BOM
...
## [2026-04-13] BUG-03: PermissionError при загрузке media на prod

**Проблема:** `PermissionError: [Errno 13] Permission denied: 'media/3'`
при POST /api/projects/{id}/media на prod.

**Контекст:** Dockerfile.prod создаёт `/media` (абсолютный путь) и
выставляет owner=appuser, но backend по умолчанию использует `./media`
(т.е. `/app/media/`). При docker build `/app/media/` создаётся из
COPY с owner=root.

**Решение:**
1. Оперативно: `docker exec -u root backend chown -R appuser:appuser /app/media`
2. Долгосрочно: в Dockerfile.prod добавить `mkdir -p /app/media` в RUN

**Урок:** Относительные пути в config (./media) при WORKDIR /app резолвятся
в /app/media, а не в /media. Dockerfile должен создавать обе директории.

---

## [2026-04-13] BUG-04: OBPPC POST 422 validation error

**Проблема:** Создание записи OBPPC возвращало 422 Unprocessable Entity.

**Контекст:** Frontend отправлял `pack_size_ml: Number("")` = 0, но
schema имеет `ge=1`. Также `price_point: ""` не парсится в Decimal.

**Решение:** Sanitize на frontend: `Number(packSize) > 0 ? Number(packSize) : null`
и `pricePoint.trim() !== "" ? pricePoint : null`.

**Урок:** Числовые поля с `ge=1` в Pydantic отклоняют 0. При конвертации
пустой строки в Number() получается 0, не null. Всегда sanitize перед отправкой.

---

## [2026-04-13] BUG-05: Ошибка в сценариях при частичном расчёте

**Проблема:** Если один из 3 сценариев не рассчитан (404), loadAll()
пробрасывал ошибку и UI показывал общее "Ошибка" для всех сценариев.

**Контекст:** В цикле загрузки результатов для не-404 ошибок делался
`throw err`, что ломало весь loadAll и скрывало результаты остальных
рассчитанных сценариев.

**Решение:** Все ошибки в цикле загрузки результатов ловятся и не
пробрасываются — сценарий просто помечается как не рассчитанный.

**Урок:** При загрузке данных по нескольким сущностям — fail-tolerant подход,
не ломать весь UI из-за одного missing ресурса.
