...
### D-12 — Excel quirk: NPV/ROI/IRR scope "Y1-Y5" использует 6 элементов

**Excel (DATA row 48 col 3 — NPV "Y1-Y5"):**
```
=SUM(B44:G44)
```
Диапазон `B:G` = 6 столбцов (B,C,D,E,F,G), что соответствует **6 годам**
(year_label 0..5), а не 5 как подразумевает label "Y1-Y5".

Аналогичная проблема в формулах ROI Y1-Y5 и IRR Y1-Y5:
```
ROI: =(-SUM(B43:G43)/(SUMIF(B43:G43,"<0")-1))/COUNT(B43:G43)
IRR: =IRR(B43:G43)
```

Сравнение с другими scope'ами в той же ячеечной структуре:
- Y1-Y3: `B:D` = 3 столбца ✓ (3 года, согласуется с label)
- Y1-Y5: `B:G` = 6 столбцов ❌ (6 лет, рассогласовано с label "5")
- Y1-Y10: `B:K` = 10 столбцов ✓ (10 лет, согласуется с label)

**Что это:** скорее всего typo автора Excel — должно быть `B:F`
(5 столбцов = 5 лет). Но Excel-цифры в KPI sheet, бизнес-документации
и презентациях рассчитаны и отражают именно 6-элементный диапазон.

**Решение для реализации:**
По ADR-CE-01 (Excel = источник истины) реализуем как в Excel —
6 элементов для скоупа Y1-Y5. Зафиксировано в `s11_kpi.py`:
```python
SCOPE_BOUNDS = {
    "y1y3":  (3,  3),
    "y1y5":  (6,  5),    # Excel quirk: slice 6 элементов, threshold 5 лет
    "y1y10": (10, 10),
}
```

Если позже бизнес скажет "это была опечатка, Y1-Y5 должно быть 5 лет" —
правка в одной строке `SCOPE_BOUNDS`. Все остальные KPI (NPV/IRR/ROI/payback)
для скоупа Y1-Y5 при этом изменятся.

**Численная разница для GORJI:**
- 6 элементов (Excel): NPV Y1-Y5 = 27.25M, IRR = 64.1%, ROI = 67.4%
- 5 элементов (label): NPV Y1-Y5 = 13.59M, IRR ≈ 47%, ROI ≈ 72%
