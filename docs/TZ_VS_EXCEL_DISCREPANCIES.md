...
**Статус:** ✅ Финально закрыт 2026-05-15. Excel-тайпо **отклонён**:
скоуп Y1-Y5 = 5 лет (60 мес) — `SCOPE_BOUNDS["y1y5"] = (5, 5)`.

**История:**
- 2026-04-08: openpyxl подтвердил, что в Excel-эталоне формула
  `C48 = SUM(B44:G44)` суммирует 6 столбцов вместо 5 — typo автора
  Excel в одной формуле.
- 2026-04-13: заказчик одобрил правку 6→5 (финансовая корректность
  важнее точного следования Excel).
- 2026-05-15: MEMO v2.1 7.1.3 предлагал вернуться к 6 столбцам
  ("не все SKU стартуют с Y1M1"). Заказчик отклонил, подтвердил
  семантику "Y1-Y5 = с начала проекта по конец 5-го года включительно
  (60 мес)".

Полное обоснование Excel-тайпо ниже сохранено как историческая
справка — текущая реализация в коде уже учитывает 5 лет.
...
#### Финальное решение (2026-05-15)

Заказчик отклонил Excel-тайпо. Скоуп Y1-Y5 = 5 лет (с начала проекта
по конец 5-го года включительно = 60 мес). Это исключение из ADR-CE-01
"Excel = источник истины" — обоснование: одна формула в Excel содержит
typo, не дизайн-решение; заказчик два раза подтвердил финансовую
семантику (2026-04-13 и 2026-05-15).

Зафиксировано в `backend/app/engine/steps/s11_kpi.py`:
```python
SCOPE_BOUNDS = {
    "y1y3":  (3,  3),
    "y1y5":  (5,  5),    # ← D-12 финально: 5 лет (60 мес), Excel-тайпо отклонён 2026-05-15
    "y1y10": (10, 10),
}
```

Acceptance-тесты пересчитаны через openpyxl на скорректированных
формулах и зафиксированы как эталон.
