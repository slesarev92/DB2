...
### Контекст
В процессе верификации (документ `docs/TZ_VS_EXCEL_DISCREPANCIES.md`) выявлены расхождения между ТЗ и рабочей Excel-моделью `ПАСПОРТ МОДЕЛЬ GORJI+ 05-09-25.xlsx`. Excel-модель используется в production и даёт корректные результаты — её цифры (NPV, IRR, ROI, Payback) проверены бизнесом.

### Решение
**Excel-модель `ПАСПОРТ МОДЕЛЬ GORJI+ 05-09-25.xlsx` является единственным источником истины для всех финансовых формул расчётного ядра.**
