...
**Polza AI** (`https://api.polza.ai/api/v1`) как единая точка доступа
к LLM:

- **Совместимость:** 100% OpenAI API. Используем `openai` Python SDK
  (`AsyncOpenAI` с кастомным `base_url`) — никаких ad-hoc HTTP-клиентов.
- **Оплата:** в рублях по корпоративному счёту, без VPN, без валютных операций.
- **Маршрутизация моделей:** Polza проксирует на провайдеров под капотом.
  Формат `model` параметра — `"<provider>/<model_id>"`:
  - `"anthropic/claude-sonnet-4-6"` — **дефолт** для обычных задач
    (генерация executive summary, комментарии к графикам, объяснения
    дельт сценариев)
  - `"anthropic/claude-opus-4-6"` — **опция** для сложных задач
    (аудит финансовых формул, валидация интерпретации, ответы на
    open-ended вопросы аналитика). Включается per-endpoint флагом
    или per-environment настройкой
  - При необходимости — `"openai/gpt-4o"` как fallback (тот же SDK,
    та же сигнатура)

**Конфигурация (`.env.example`):**
```
POLZA_AI_API_KEY=<секрет, выдаётся в кабинете Polza>
POLZA_AI_BASE_URL=https://api.polza.ai/api/v1
```

**Модуль (создаётся в задаче 7.1, не сейчас):**
```
backend/app/services/ai_service.py    # тонкий wrapper над AsyncOpenAI
backend/app/api/ai.py                  # endpoint(s) /api/projects/{id}/ai/*
```

**Зависимости (добавляются в requirements.txt в Фазе 7, не сейчас):**
```
openai>=1.0
```
...
| ADR-16 | AI-интеграция | Polza AI (OpenAI-совместимый прокси), Claude 4.6 Sonnet/Opus, Фаза 7 post-MVP | Финально |
