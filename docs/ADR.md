...
**Base URL'ы (verified из docs/api-reference 2026-04-09):**
- **`https://polza.ai/v1`** — для openai SDK (chat completions, images,
  embeddings — стандартные OpenAI-compat endpoints). **ЭТО используем.**
- `https://polza.ai/api` — native Polza API (universal `/media` endpoint
  для cross-modal генерации, тонкая обёртка). Опциональный fallback.

⚠️ **Note:** В первой версии этого ADR (2026-04-09) был указан URL
`https://api.polza.ai/api/v1` (со своим subdomain `api.`) — это **было
неточно**. После проверки документации (Polza docs `/docs/llms.txt`,
`/docs/api-reference/introduction`) правильный URL для openai SDK —
`https://polza.ai/v1` без subdomain. .env.example исправлен.
...
**1. Chat / text completions (`/v1/chat/completions`):**
- 400+ моделей из 70+ провайдеров (Anthropic, OpenAI, Google, DeepSeek,
  Meta, Yandex, etc)
...
POLZA_AI_BASE_URL=https://polza.ai/v1
```
...
| ADR-16 | AI-интеграция | Polza AI (`https://polza.ai/v1`, OpenAI-совместимый), Claude 4.6 Sonnet/Opus chat + Flux-2-pro image gen + web search, Фаза 7 post-MVP | Финально |
